...
const OPENROUTER_USAGE_DIR = join(homedir(), ".1code-openrouter", "sessions")
...
export interface OpenRouterTodayUsage {
  tokens: number
  inputTokens: number
  outputTokens: number
  sessions: number
...
async function readSessionTotals(filePath: string): Promise<SessionTotals> {
  ...
  for (const line of content.split("\n")) {
    ...
    totals.costUsd += Math.max(0, cost)
    totals.hasUsage = true
  }
  return totals
}

export async function readOpenRouterToday(): Promise<OpenRouterTodayUsage | null> {
  if (!existsSync(OPENROUTER_USAGE_DIR)) {
    return null
  }
  const empty: OpenRouterTodayUsage = {
    tokens: 0,
    inputTokens: 0,
    outputTokens: 0,
    sessions: 0,
